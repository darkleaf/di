...
- **Visualizing your system** — новая страница. Берём пример из
  ...
  **Готово:** `test/darkleaf/di/how_to/visualizing_test.clj`.
  Функцию `graphviz` дали в теле рецепта (не в core API),
  минимальный `digraph` в форме `{ deps } -> key`. PR #30 в
  core не мержим — там та же логика плюс закомментированные
  наработки (кластеры по неймспейсам, подсветка корня).
